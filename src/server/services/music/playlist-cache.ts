...
import { fetchSpotifyPlaylistData, fetchSpotifyPlaylistSnapshotId } from './spotify';
...
const derivePlaylistFromSpotify = (playlistId: string) =>
  Effect.gen(function* (_) {
    ...
    const tracks = spotifyData.tracks.map((track, index): DeezerTrackPayload => {
...
export const getDerivedPlaylist = (playlistId: string) =>
  Effect.gen(function* (_) {
    const cachedEntryOption = yield* _(spotifyDerivedPlaylistCache.getOption(playlistId));

    if (Option.isSome(cachedEntryOption)) {
      const cachedEntry = cachedEntryOption.value;
      const latestSnapshotId = yield* _(fetchSpotifyPlaylistSnapshotId(playlistId));

      if (latestSnapshotId !== cachedEntry.snapshotId) {
        yield* _(spotifyDerivedPlaylistCache.invalidate(playlistId));
        const refreshedEntry = yield* _(spotifyDerivedPlaylistCache.get(playlistId));
