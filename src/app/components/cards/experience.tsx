...
export function Experience({ experience }: ExperienceProps) {
  ...
  useEffect(() => {
    ...
    const measuredHeights = descriptionRefs.current.map(
      (node) => node?.scrollHeight ?? 0,
    );
      ...
      <div className={"flex items-center justify-between gap-4"}>
        ...
        {hasDescriptions && (
          <Button
            ...
            aria-label={
              isExpanded ? t("toggle.showLess") : t("toggle.showMore")
            }
            className={"px-2"}
          ...
          >
            {isExpanded ? (
              <ChevronsDownUpIcon
                className={"text-ink-muted size-4 stroke-3"}
              />
            ) : (
              <ChevronsUpDownIcon
                className={"text-ink-muted size-4 stroke-3"}
          ...
          return (
            ...
            >
              <div className={"flex flex-col items-center gap-2"}>
                <span
                  className={
                    "bg-divider mt-2 h-2 w-2 flex-shrink-0 rounded-full"
                  }
                />
                {!isLast && (
